...
const buildMock = { getCurrentTheme: jest.fn(() => theme) }
jest.setMock('../../theme/buildTheme', buildMock)
...
describe('getTheme', () => {
  describe('getTheme', () => {
    ...
    it('should call getCurrentTheme to get the latest theme', () => {
      expect(buildMock.getCurrentTheme).not.toHaveBeenCalled()

      getTheme([ 'components', 'button', 'default' ], {})

      expect(buildMock.getCurrentTheme).toHaveBeenCalled()
    })
  })
})
