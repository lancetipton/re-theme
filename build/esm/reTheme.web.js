...
var removeThemeEvent = function removeThemeEvent(event, removeListener) {
  ...
  : isFunc(removeListener) && isArr(listeners$1[event]) && (listeners$1[event] = listeners$1[event].filter(function (listener) {
    return listener !== removeListener;
  }));
};
...
var hookFactory = function hookFactory(events) {
  return (
    function (offValue, onValue) {
      ...
      var _useState3 = useState(checkJoinValues(offValue, onValue, onValue, noMerge)),
